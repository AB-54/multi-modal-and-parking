...
CLASSIFICATION_CODES = {"ABP": ("CT08", "CT03"), "VOA SCAT": (None,)}
# Most important thing to edit. Working with multiple ABP codes. Wildcards automatically added (so "CT03" also
# captures listings of "CT03PP"). VOA SCAT typically left as "None", but should work if you want to include it.

WAREHOUSE_ABP_CODE = ("CT03PP")
# This is just used to summarise the sub-codes of a given ABP code. Works with only one code. Does not need to be
# edited.

POSTCODE_AREA = "M20%"
# Not working perfectly, as if "M1" is chosen then it also captures postcodes like "CM1". AB working on fix.

##### CLASSES #####

##### FUNCTIONS #####
def to_kepler_geojson(geodata: gpd.GeoDataFrame, out_file: pathlib.Path | None) -> None:
    """Convert to Web mercator CRS and output to GeoJSON. You may not need this if you are not planning on having
    a map displaying your outputs.
    ...
    """
    if out_file is None:
        LOG.info("No output file specified. GeoJSON will not be saved.")
        return
...
def voa_code_count(
    ...
    """Count rows with VOA and ABP codes and output summary spreadsheet. Not strictly necessary for analysis.
...
def get_warehouse_positions(
    ...
    """Select warehouse data and join UPRN coordinates to this.
    ...
    -------
    gpd.GeoDataFrame
        GeoDataFrame containing warehouse data with coordinates.
    """
    query = """
    SELECT q.*, blpu.x_coordinate, blpu.y_coordinate, blpu.latitude, blpu.longitude, org.organisation, sd.street_description
    ...
    LEFT JOIN data_common.abp_organisation org ON q.uprn = org.uprn
    LEFT JOIN data_common.abp_lpi lpi ON q.uprn = lpi.uprn
    LEFT JOIN data_common.abp_street_descriptor sd ON lpi.usrn = sd.usrn
    """
    ...
    # # TODO (AB)- Add the mm_multistorey data. This is a work in progress
    # multistorey_query = """
    # SELECT *
    # FROM data_common.mm_multistorey
    # LEFT JOIN data_common.abp_crossref ON mm_multistorey.toid = abp_crossref.cross_reference
    # LEFT JOIN data_common.abp_classification ON abp_classification.uprn = abp_crossref.uprn
    # WHERE class_scheme LIKE 'AddressBase Premium Classification Scheme';
    # """
    # multistorey_data = connected_db.query_to_dataframe(multistorey_query)
    #
    # # Convert the geometry from WKT to Shapely geometry
    # multistorey_geom = gpd.GeoSeries.from_wkt(multistorey_data["geom_wkt"])
    # multistorey_gdf = gpd.GeoDataFrame(multistorey_data, geometry=multistorey_geom)
    #
    # # Merge the multistorey data with the warehouse data based on 'uprn' column
    # data = pd.merge(data, multistorey_gdf, on="uprn", how="left")

    return _positions_geodata(data, output_file)


# def warehouse_locations_query(
#         postcode: str,
#         voa_scat: Sequence[str] | None = None,
#         abp: Sequence[str] = None,
#         year: int | None = None,
# ) -> sql.Composable:
#     """Select warehouse data for given postcode and include location information.
#
#     Parameters
#     ----------
#     postcode : str
#         Postcode to filter warehouses on.
...
#     """
#     query = r"""
#     SELECT cl.*, b.postcode_locator, org.organisation
#     FROM (
#         {abp_classification}
#     ) cl
#     JOIN (
#         SELECT uprn, postcode_locator
#         FROM data_common.abp_blpu
#         WHERE postcode_locator ILIKE {postcode}
#     ) b ON cl.uprn = b.uprn
#     """
#
#     # if abp is None:
#     #     abp = WAREHOUSE_ABP_CODE
#
#     return sql.SQL(query).format(
#         abp_classification=classification_codes_query(voa_scat, abp, year),
#         postcode=sql.Literal(f"{postcode}"),
#     )
...
def classification_codes_query(
    ...
    # Add the '%' wildcard to each classification code in the ABP list
    abp_with_wildcard = [code + "%" for code in abp]
    ...
    WHERE (
        ...
        ) OR classification_code LIKE ANY (ARRAY[{abp_with_wildcard}])
    )
    """
    sql_query = sql.SQL(query.strip()).format(
        scat=sql.SQL(",").join(sql.Literal(i) for i in voa_scat),
        abp_with_wildcard=sql.SQL(",").join(sql.Literal(i) for i in abp_with_wildcard),
    )
...
def warehouse_organisations_query(
    ...
    """Select warehouse data for given postcode.
...
def get_classification_codes(connected_db: database.Database, output_folder: pathlib.Path) -> None:
    """Select ABP classification codes from the database. This is used to simply output a .csv describing the codes
    selected in WAREHOUSE_ABP_CODE and their derivatives, it is not strictly important for analysis"""

    if WAREHOUSE_ABP_CODE[0] == "%":
        LOG.warning("Invalid warehouse ABP code: %s", WAREHOUSE_ABP_CODE[0])
        return

    warehouse_abp_code = WAREHOUSE_ABP_CODE[0].replace("%", "")  # Remove '%' from the code

    primary_code = warehouse_abp_code[0]
    secondary_code = warehouse_abp_code[1]
    tertiary_code = warehouse_abp_code[2:].lstrip('0')  # Remove leading zeros
    ...
    WHERE "Primary_Code" = '{primary_code}' AND "Secondary_Code" = '{secondary_code}'
        AND "Tertiary_Code" = '{tertiary_code}'
...
def csv_creator(
    ...
    """Run select `query` and provides .csv output.
    ...
    -------
    pd.DataFrame
        Positions dataframe with columns: "uprn", "classification_code", "postcode_locator", "street_description",
        "x_coordinate", "y_coordinate", "area", "organisation".
    """
    positions = get_warehouse_positions(
        connected_db,
        query,
    ...
    ### TODO (For AB) - match with stations, change to lat/long?? Maybe

    # Join positions and floorspace based on 'uprn' column
    positions = pd.merge(positions, floorspace[['uprn', 'area']], on='uprn', how='left')

    return positions[
        ["uprn", "classification_code", "postcode_locator", "street_description", "x_coordinate", "y_coordinate",
         "area", "organisation", "latitude", "longitude"]]


def combine_lsoa_areas(lsoa_data: list[pd.DataFrame], lsoa_id_column: str, output_file: pathlib.Path) -> None:
    ...
    lsoa_warehouse = lsoa_warehouse.drop_duplicates("uprn")
    after = len(lsoa_warehouse)
    difference = before - after

    if before == 0:
        msg = "No rows found in lsoa_warehouse"
    else:
        msg = f"Dropped {difference:,.0f} ({difference / before:.1%}) rows with duplicate UPRNs ({after:,.0f} remaining)"
    LOG.info(msg)

    lsoa_summary = lsoa_warehouse.groupby(lsoa_id_column).agg(warehouse_count=('uprn', 'size'),
                                                              area_total=('area', 'sum')).reset_index()
    lsoa_summary.to_csv(output_file, index=False)
    LOG.info("Written combined warehouse floorspace: %s", output_file)


def lsoa_areas_no_agg(lsoa_data: list[pd.DataFrame], output_file: pathlib.Path) -> None:
    """Combine the output from `warehouse_by_lsoa` without aggregating to LSOA.
...
def extract_warehouses(
    ...
    with database.Database(database_connection_parameters) as connected_db:
        ...
        queries = {
            "warehouses_postcode": warehouse_organisations_query(POSTCODE_AREA, year=year),
        }

        lsoa_warehouse_floorspace: list[pd.DataFrame] = []
        positions_data: list[pd.DataFrame] = []
        ...
        if year is None:
            out_file = output_folder / "warehouse_floorspace_per_location.csv"
        else:
            out_file = (
                    output_folder / f"warehouse_floorspace_{year}_per_location.csv"
            )

        combine_lsoa_areas(lsoa_warehouse_floorspace, shapefile.id_column, out_file)

        lsoa_areas_no_agg(positions_data, out_file.with_name(out_file.stem + "_positions.csv"))
